...
{
	Player* player = (Player*)CharacterManager::Instance()->get_player();
	Vector2 pos_cnter = player->get_logic_center();
	...
	case Player::AttackDir::Left:
		pos_hit_box = Vector2(pos_cnter.x - size_hit_box.x / 2, pos_cnter.y);
		break;
    case Player::AttackDir::Right:
        pos_hit_box = Vector2(pos_cnter.x + size_hit_box.x / 2, pos_cnter.y);
        break;
	case Player::AttackDir::Up:
		pos_hit_box = Vector2(pos_cnter.x, pos_cnter.y - size_hit_box.y / 2);
		break;
