...
void CollisionManager::process_collide()
{
	for (CollisionBox* collision_box_src : collision_box_list)
	{
		...
		{
			if (!collision_box_dst->enabled || collision_box_src == collision_box_dst || collision_box_src->layer_dst != collision_box_dst->layer_dst)
				continue;
...
void CollisionManager::on_debug_render()
{
	...
	{
		setlinecolor(collision_box->enabled ? RGB(255, 195, 0) : RGB(115, 115, 175));
